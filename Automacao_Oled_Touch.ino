...
void cmdControl(const unsigned int rawCmd[]);
...
void cmdControl(const unsigned int rawCmd[]){
  for (byte i=0; i<59; i++){
    cmd[i] = pgm_read_word_near(rawCmd + i);
    //Serial.print(String(cmd[i])+", ");
  }
  return cmd;
} 